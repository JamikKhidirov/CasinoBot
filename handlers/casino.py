...
async def init_db():
    ...
    try:
        ...
        await conn4.close()
    # migration: add blackjack_balance column for existing DBs
    conn5 = await get_db()
    try:
        await conn5.execute("ALTER TABLE users ADD COLUMN blackjack_balance INTEGER DEFAULT 1000")
    except:
        pass
    finally:
        await conn5.close()
...
async def cb_casino_profile(call: CallbackQuery):
    ...
    bj_bal = user["blackjack_balance"] if user["blackjack_balance"] else INITIAL_BLACKJACK_BALANCE
    text = (
        f"<b>📊 Профиль игрока</b> {call.from_user.first_name}\n\n"
        f"┃ 🆔 ID: <code>{user['user_id']}</code>\n"
        f"┃ 💰 <b>Баланс (PVP):</b> {user['balance']} 🪙\n"
        f"┃ 🃏 <b>Баланс (блэкджек):</b> {bj_bal} 🪙\n"
...
async def cmd_blackjack(message: Message):
    ...
    bal = user["blackjack_balance"] if user["blackjack_balance"] else INITIAL_BLACKJACK_BALANCE
    if bal < bet:
...
async def cb_bj_join(call: CallbackQuery):
    ...
    async with active_games_lock:
        ...
        user = await get_user(call.from_user.id)
        if not user or (user.get("blackjack_balance") or INITIAL_BLACKJACK_BALANCE) < game.bet:
